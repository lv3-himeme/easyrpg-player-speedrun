...
void Window_Interpreter::DrawRuntimeFlagsWindow() const {
	...
	for (auto info : runtime_flags) {
		...
		int r = 16;
		contents->TextDraw(sub_window_flags->GetWidth() - r, rect.y, Font::ColorCritical, is_overridden ? "[BẬT]" : "[TẮT]", Text::AlignRight);
		r += 3 * 8;
		if (!is_overridden)
			r += 8;
		contents->TextDraw(sub_window_flags->GetWidth() - r, rect.y, Font::ColorDefault, "->", Text::AlignRight);
		r += 16;
		contents->TextDraw(sub_window_flags->GetWidth() - r, rect.y, Font::ColorDisabled, is_active ? "[BẬT]" : "[TẮT]", Text::AlignRight);
